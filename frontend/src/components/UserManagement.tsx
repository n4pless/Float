...
const SectionLabel: React.FC<{ label: string; right?: React.ReactNode }> = ({ label, right }) => (
  <div className="flex items-center justify-between px-4 py-2.5 border-b border-drift-border bg-drift-panel">
    <span className="text-[13px] font-semibold text-txt-0 uppercase tracking-wider">{label}</span>
    {right}
  </div>
);
...
const HealthGauge: React.FC<{ value: number; size?: number }> = ({ value, size = 72 }) => {
  ...
  const color = pct > 50 ? '#00D26A' : pct > 20 ? '#F0B90B' : '#FF4D6A';
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[20px] font-bold font-mono tabular-nums text-txt-0 leading-none">{pct.toFixed(0)}</span>
        <span className="text-[9px] text-txt-1 mt-0.5">HEALTH</span>
      </div>
    </div>
  );
};
        ...
        <div className="w-full max-w-5xl mx-auto px-4 py-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-4">
              ...
              <div className="border border-drift-border overflow-hidden">
                <SectionLabel label="Wallet"
                  right={
                    <div className="relative" ref={faucetRef}>
                      ...
                      {faucetOpen && (
                        <div className="absolute right-0 top-full mt-1 z-20 bg-drift-surface border border-drift-border py-1 min-w-[140px]">
                  ...
                  <div className="flex items-center justify-between px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-txt-1">SOL</span>
                      </div>
                    </div>
                  </div>
                  {/* USDC row */}
                  <div className="flex items-center justify-between px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-txt-1">USDC</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-4 py-2.5 border-t border-drift-border">
                  <p className="text-[10px] text-txt-1 font-mono truncate flex-1">{pubkeyStr}</p>
                  <button onClick={handleCopy}
                    className="p-1 text-txt-1 hover:text-txt-0 hover:bg-drift-surface transition-colors"
                    title="Copy address">
                ...
                {isUserInitialized ? (
                  <>
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="flex items-center h-10 bg-drift-input border border-drift-border focus-within:border-txt-3/40 transition-colors" style={{ borderRadius: 4 }}>
                      ...
                      {!confirmStep ? (
                        ...
                        <div className="space-y-2">
                          <div className="bg-drift-input border border-drift-border px-3 py-2.5">
                            <div className="flex items-center justify-between">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <button onClick={() => setConfirmStep(false)}
                              className="h-10 text-[11px] font-semibold border border-drift-border text-txt-1 hover:bg-drift-surface transition-colors" style={{ borderRadius: 6 }}>
                              Cancel
                            </button>
                            <button
                              onClick={activeTab === 'deposit' ? handleDeposit : handleWithdraw}
                              disabled={!!loading}
                              className={`h-10 text-[12px] font-semibold transition-colors disabled:opacity-30 flex items-center justify-center gap-1.5 ${
                                activeTab === 'deposit'
                    ...
                    <div className="p-4 space-y-3">
                      <div className="space-y-1.5">
                        ...
                        <input type="text" value={accountName} onChange={e => setAccountName(e.target.value)}
                          ...
                          className="w-full h-9 px-3 bg-drift-input border border-drift-border text-[12px] text-txt-0 placeholder:text-txt-3/40 outline-none focus:border-txt-3/40 transition-colors" style={{ borderRadius: 4 }} />
                      </div>
                      <div className="space-y-1.5">
                        <label className="text-[10px] text-txt-3">Deposit Amount (USDC)</label>
                        <div className="flex items-center h-9 bg-drift-input border border-drift-border focus-within:border-txt-3/40 transition-colors" style={{ borderRadius: 4 }}>
            ...
            <div className="space-y-4">
                        ...
                        <div key={acct.subAccountId}
                          ...
                          <div className="h-1 rounded-full bg-drift-surface overflow-hidden">
                            <div className="h-full rounded-full transition-all duration-500"
                              style={{
                                width: `${acctHealth}%`,
                                background: acctHealth > 50 ? '#00D26A' : acctHealth > 20 ? '#F0B90B' : '#FF4D6A',
                              }} />
                          </div>
                        </div>
                      );
                  ...
                  {showCreateSub && (
                    <div className="border-t border-drift-border p-4 space-y-2 bg-drift-bg/40">
                      <input type="text" value={accountName} onChange={e => setAccountName(e.target.value)}
                        ...
                        className="w-full h-8 px-3 bg-drift-input border border-drift-border text-[11px] text-txt-0 placeholder:text-txt-3/40 outline-none focus:border-txt-3/40 transition-colors" style={{ borderRadius: 4 }} />
                      <div className="flex items-center gap-2">
                        <div className="flex items-center flex-1 h-8 bg-drift-input border border-drift-border focus-within:border-txt-3/40 transition-colors" style={{ borderRadius: 4 }}>
                          <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
              ...
              <div className="border border-drift-border overflow-hidden flex-1">
          ...
          {!isUserInitialized && (
            <div className="border border-drift-border bg-drift-panel px-4 py-8 text-center">
        ...
        <DialogContent>
          ...
          <DialogFooter>
            <button onClick={() => setShowDeleteDialog(false)} disabled={isDeleting}
              className="px-4 py-2 text-[11px] font-semibold border border-drift-border text-txt-1 hover:bg-drift-surface transition-colors disabled:opacity-50" style={{ borderRadius: 6 }}>
              Cancel
            </button>
            <button onClick={() => selectedSubAccountForDeletion !== null && handleDeleteUser(selectedSubAccountForDeletion)}
              disabled={isDeleting}
              className="px-4 py-2 text-[11px] font-semibold bg-bear text-white hover:brightness-110 transition-all disabled:opacity-50 flex items-center gap-1.5" style={{ borderRadius: 6 }}>

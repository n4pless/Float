...
const SectionLabel: React.FC<{ label: string; right?: React.ReactNode }> = ({ label, right }) => (
  <div className="flex items-center justify-between px-4 py-3 border-b border-drift-border/40">
    <span className="text-[13px] font-bold text-txt-0">{label}</span>
    {right}
  </div>
);
        ...
        <div className="w-full max-w-5xl mx-auto px-4 py-5 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row items-stretch">
                ...
                <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 divide-drift-border">
                  ...
                  <div className="px-4 py-4 border-b sm:border-b-0 sm:border-r border-drift-border">
                    <span className="text-[10px] text-txt-2 uppercase tracking-wider font-semibold block mb-1">Equity</span>
                    <span className="text-[20px] font-bold font-mono tabular-nums text-txt-0 leading-none">${equity.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  {/* Free Collateral */}
                  <div className="px-4 py-4 border-b sm:border-b-0 sm:border-r border-drift-border">
                    <span className="text-[10px] text-txt-2 uppercase tracking-wider font-semibold block mb-1">Free Collateral</span>
                    <span className="text-[20px] font-bold font-mono tabular-nums text-txt-0 leading-none">${freeCollateral.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  {/* Leverage */}
                  <div className="px-4 py-4 border-b sm:border-b-0 sm:border-r border-drift-border">
                    <span className="text-[10px] text-txt-2 uppercase tracking-wider font-semibold block mb-1">Leverage</span>
                    <span className="text-[20px] font-bold font-mono tabular-nums text-txt-0 leading-none">{leverage.toFixed(2)}&times;</span>
                    <span className="text-[10px] text-txt-2 uppercase tracking-wider font-semibold block mb-1">Unrealized P&L</span>
                    <div className="flex items-baseline gap-2">
                      <span className={`text-[20px] font-bold font-mono tabular-nums leading-none ${unrealizedPnl >= 0 ? 'text-bull' : 'text-bear'}`}>
                ...
                {/* Health bar */}
                <div className="flex items-center px-5 py-4 border-t sm:border-t-0 sm:border-l border-drift-border bg-drift-bg/40">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {!confirmStep ? (
                        <button onClick={() => {
                          ...
                          className={`w-full h-10 text-[12px] font-semibold rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed ${
                            activeTab === 'deposit'
            ...
            <div className="space-y-4">
                        ...
                        <div key={acct.subAccountId}
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              {isActive && (
                                <span className="px-1.5 py-px text-[9px] font-bold rounded-full bg-drift-surface text-txt-0 border border-drift-border shrink-0">Active</span>
                              )}
                            </div>
                          ...
                          <div className="h-1 rounded-full bg-drift-surface overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-500 ${acctHealth > 50 ? 'bg-bull' : acctHealth > 20 ? 'bg-accent' : 'bg-bear'}`}
                              style={{ width: `${acctHealth}%` }} />
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="border border-drift-border/60 rounded-xl bg-drift-panel/80 overflow-hidden flex-1">
